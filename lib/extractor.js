...
div[class*='date']`);
    return (
      __guard__(
        cleanNull(
          __guard__(
            dateCandidates != null ? dateCandidates.first() : undefined,
            (x1) => x1.attr('content')
          )
        ),
        (x) => x.trim()
  ...
  copyright(doc) {
    const copyrightCandidates =
      doc(`p[class*='copyright'], div[class*='copyright'], span[class*='copyright'], li[class*='copyright'], \
...
meta[name='creator']`);
    ...
    authorCandidates.each(function () {
      const author = __guard__(
        cleanNull(__guard__(doc(this), (x1) => x1.attr('content'))),
        (x) => x.trim()
      );
      if (author) {
    ...
    if (authorList.length === 0) {
      const fallbackAuthor =
        __guard__(doc("span[class*='author']").first(), (x) => x.text()) ||
        __guard__(doc("p[class*='author']").first(), (x1) => x1.text()) ||
        __guard__(doc("div[class*='author']").first(), (x2) => x2.text()) ||
        __guard__(doc("span[class*='byline']").first(), (x3) => x3.text()) ||
        __guard__(doc("p[class*='byline']").first(), (x4) => x4.text()) ||
        __guard__(doc("div[class*='byline']").first(), (x5) => x5.text());
      if (fallbackAuthor) {
...
meta[name='DC.Publisher']`);
    return (
      __guard__(
        cleanNull(
          __guard__(
            publisherCandidates != null
              ? publisherCandidates.first()
              : undefined,
            (x1) => x1.attr('content')
          )
        ),
...
meta[name='twitter:image0']`);
  ...
  favicon(doc) {
    const tag = doc('link').filter(function () {
      return (
        __guard__(doc(this).attr('rel'), (x) => x.toLowerCase()) ===
        'shortcut icon'
      );
    });
  ...
  description(doc) {
    ...
    return __guard__(
      cleanNull(
        __guard__(tag != null ? tag.first() : undefined, (x1) =>
          x1.attr('content')
        )
      ),
      (x) => x.trim()
...
const firstTitleChunk = function (title, splitter) {
  return title.split(splitter).shift()
};
...
var getSiblingsContent = function (
  doc,
  lang,
  currentSibling,
  baselinescoreSiblingsPara
...
const isNodescoreThresholdMet = function (doc, node, e) {
  ...
  if (
    ...
    !['td', 'ul', 'ol', 'blockquote'].includes(e[0].name)
  ) {
    return false;
  } else {
    return true;
...
var cleanNull = (text) =>
  text != null ? text.replace(/^null$/g, '') : undefined;
...
var cleanTitle = function (title, delimiters) {
  ...
  each(delimiters, function (c) {
    if (titleText.indexOf(c) >= 0 && !usedDelimeter) {
      titleText = firstTitleChunk(titleText, c);
...
var rawTitle = function (doc) {
  ...
  each(
    [
      __guard__(
        __guard__(doc("meta[property='og:title']"), (x1) => x1.first()),
        (x) => x.attr('content')
      ),
      __guard__(
        __guard__(doc("h1[class*='title']"), (x3) => x3.first()),
        (x2) => x2.text()
      ),
      __guard__(
        __guard__(doc('title'), (x5) => x5.first()),
        (x4) => x4.text()
      ),
      __guard__(
        __guard__(doc('h1'), (x7) => x7.first()),
        (x6) => x6.text()
      ),
      __guard__(
        __guard__(doc('h2'), (x9) => x9.first()),
        (x8) => x8.text()
      ),
    ],
    function (candidate) {
